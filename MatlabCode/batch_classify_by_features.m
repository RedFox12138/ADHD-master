...
% 创建输出文件夹
if ~exist(output_folder_sampen_greater, 'dir')
    mkdir(output_folder_sampen_greater);
    fprintf('创建文件夹: %s\n', output_folder_sampen_greater);
end

if ~exist(output_folder_sampen_less, 'dir')
    mkdir(output_folder_sampen_less);
    fprintf('创建文件夹: %s\n', output_folder_sampen_less);
end
...
% 统计变量
count_sampen_greater = 0;  % 静息样本熵 > 注意力样本熵的文件数
count_sampen_less = 0;     % 静息样本熵 < 注意力样本熵的文件数
count_processed = 0; % 成功处理的文件数
count_failed = 0;    % 处理失败的文件数

% 创建结果记录表
results = cell(n_files, 4);
results(1, :) = {'文件名', '静息样本熵均值', '注意力样本熵均值', '样本熵分类'};
...
for file_idx = 1:n_files
    ...
    try
        ...
        if mean_rest_sampen > mean_attention_sampen
            dest_file = fullfile(output_folder_sampen_greater, filename);
            copyfile(filepath, dest_file);
            count_sampen_greater = count_sampen_greater + 1;
            sampen_class = '静息>注意力';
            fprintf('  ✓ 静息样本熵 > 注意力样本熵，已复制到: 静息样本熵大于注意力\n');
        % 样本熵分类: 静息 < 注意力
        elseif mean_rest_sampen < mean_attention_sampen
            dest_file = fullfile(output_folder_sampen_less, filename);
            copyfile(filepath, dest_file);
            count_sampen_less = count_sampen_less + 1;
            sampen_class = '静息<注意力';
            fprintf('  ✓ 静息样本熵 < 注意力样本熵，已复制到: 静息样本熵小于注意力\n');
        else
            sampen_class = '静息=注意力';
            fprintf('  - 静息样本熵 = 注意力样本熵 (%.4f)，不分类\n', mean_rest_sampen);
        end
        
        % 记录结果
        results(file_idx + 1, :) = {filename, mean_rest_sampen, mean_attention_sampen, sampen_class};
...
fprintf('成功处理: %d\n', count_processed);
fprintf('处理失败: %d\n', count_failed);
fprintf('静息样本熵 > 注意力样本熵: %d 个文件\n', count_sampen_greater);
fprintf('静息样本熵 < 注意力样本熵: %d 个文件\n', count_sampen_less);
fprintf('静息样本熵 = 注意力样本熵: %d 个文件\n', count_processed - count_sampen_greater - count_sampen_less);
fprintf('================================\n\n');

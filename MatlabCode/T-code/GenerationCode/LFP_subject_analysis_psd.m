...
nh3_index = [75*Fs:1:130*Fs]; % 氨气刺激后1min
...
% 添加背景色块
yl = ylim;
patch([10 70 70 10], [yl(1) yl(1) yl(2) yl(2)], 'b',...
      'FaceAlpha', 0.1, 'EdgeColor', 'none', 'DisplayName', '非注意力时段');
patch([80 140 140 80], [yl(1) yl(1) yl(2) yl(2)], 'r',...
...
hold on;
plot(time_axis(1:length(data)),data);
xlabel('Time (s)'); % 横坐标：时间（秒）
ylabel('Amplitude (mV)'); % 纵坐标：幅度（mV）
title('EEG Signal'); % 标题
grid on; % 可选：显示网格
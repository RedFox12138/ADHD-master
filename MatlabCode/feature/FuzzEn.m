...
function [Fuzz, Ps1, Ps2] = FuzzEn(Sig, varargin)
% FuzzEn  estimates the fuzzy entropy of a univariate data sequence.
% Detailed explanation in the original code.

narginchk(1,11)
Sig = squeeze(Sig);

% Ensure no NaN or Inf values in Sig
Sig(isnan(Sig) | isinf(Sig)) = 0;
...
% Handle the case where 'r' is specified with two values for 'gudermannian'
if numel(r) == 2 && strcmpi(Fx,'gudermannian')
    r = r(1);  % Use only the first value
    fprintf('Multiple values for r entered. First value used.\n');
end

N = length(Sig);
Fun = str2func(lower(Fx));
m = m + 1;  % m is incremented as per the method requirements
Sx = zeros(N,m);

% Fill the Sx matrix for embedding
for k = 1:m
    Sx(1:N-(k-1)*tau,k) = Sig(1 + (k-1)*tau:N);
end
...
% Compute distances and apply fuzzy function
for k = 2:m
    N1 = N - k * tau;
    N2 = N - (k-1) * tau;
    T2 = Sx(1:N2,1:k) - mean(Sx(1:N2,1:k),2);
    ...
    % Avoid division by zero and prevent NaN by adding a small epsilon
    Ps1(k) = sum(sum(d1)) / max(N1 * (N1 - 1), eps);
    Ps2(k-1) = sum(sum(d2)) / max(N2 * (N2 - 1), eps);
end

% Avoid log(0) by adding a small epsilon to Ps1 and Ps2
Fuzz = (log(max(Ps1(1:end-1), eps)) - log(max(Ps2, eps))) / log(Logx);
